  ...
  stages {
    ...
    stage('Tests') {
      parallel {
        stage('Firefox') {
          steps {
            sh 'vagrant ssh -c \'$(npm bin)/testem --port ${env.RANDOM} --launch Firefox ci --file tests/testem.js\''
          }
        }
        stage('Chrome') {
          steps {
            sh 'vagrant ssh -c \'$(npm bin)/testem --port ${env.RANDOM} --launch Chrome ci --file tests/testem.js\''
          }
        }
        stage('Node') {
          steps {
